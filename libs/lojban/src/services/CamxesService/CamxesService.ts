...
const camxes = require('./camxes.js');
...
function isNodeArray(node: any): node is ParseNode[] {
    return (
        Array.isArray(node) &&
        node.every(child => Array.isArray(child))
    )
}
...
export class CamxesService {
    ...
    protected convertSelbriArray(nodes: ParseNode[]) {
    ...
    protected convertSumti(node: SpreadNode) {
    ...
    protected convertParse(node: ParseNode, parens?: Parens) {
