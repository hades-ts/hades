...
async function askForBotName(): Promise<string> {
    ...
    const botNames = projects
        .filter((project) => project.projectRelativeFolder.startsWith("bots/"))
        .map((project) => project.projectRelativeFolder.replace("bots/", ""));
...
//     return FILES_TO_RENAME.map(
//         (filenames, index) => `${index === 0 ? "" : " &&"} mv ${filenames[0]} ${filenames[1]}`,
//     ).join(" ");
// }

function copyProject(ejectPath: string, botName: string) {
    // const projects = allProjects();
    // const project = projects.find((project) => project.projectRelativeFolder === `bots/${botName}`);
