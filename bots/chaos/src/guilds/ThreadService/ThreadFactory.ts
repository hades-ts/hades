...
export class ThreadFactory {
    ...
    async validateCreation(memberId: string, channelId: string) {
        ...
        const userQuota = await this.quota.userCanSpend(memberId, threadConfig.userPeriod || { days: 1 });

        if (!userQuota) {
            throw new Error("You need to wait a bit before creating another thread.");
        }

        const channelQuota = await this.quota.channelCanSpend(channelId, threadConfig.channelPeriod || { days: 1 });
